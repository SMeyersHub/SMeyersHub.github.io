 ...
 *
 */
class AtomicClock extends Clock {
	/**
	 * The basic constructor for this class that creates an AtomicClock with the input ClockType and drift for that clock.
	 ...
	 */
	public AtomicClock(ClockType clockType, double drift) {
	...
	public void display() {
		...
		System.out.println(clockType.toString() + ", atomic clock, " + "time [" + time.formattedTime() + "], total drift = " + df.format(time.getTotalDrift()));
	}
}