...
class CuckooClock extends Clock {
	/**
	 * The basic constructor for this class that creates a CuckooClock with the input ClockType and drift for that clock.
	 ...
	 */
	public CuckooClock(ClockType clockType, double drift) {
	...
	public void display() {
		...
		System.out.println(clockType.toString() + ", cuckoo clock, " + "time [" + time.formattedTime() + "], total drift = " + df.format(time.getTotalDrift()));
	}
