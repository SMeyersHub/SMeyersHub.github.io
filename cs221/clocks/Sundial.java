...
class Sundial extends Clock {
	/**
	 * The basic constructor for this class that creates a Sundial with the input ClockType and drift for that clock.
	 ...
	 */
	public Sundial(ClockType clockType, double drift) {
	...
	public void display() {
		...
		System.out.println(clockType.toString() + ", sun dial, " + "time [" + time.formattedTime() + "], total drift = " + df.format(time.getTotalDrift()));
	}
