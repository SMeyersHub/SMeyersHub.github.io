...
class WristWatch extends Clock {
	/**
	 * The basic constructor for this class that creates a WristWatch with the input ClockType and drift for that clock.
	 ...
	 */
	public WristWatch(ClockType clockType, double drift) {
	...
	public void display() {
		...
		System.out.println(clockType.toString() + ", wrist watch, " + "time [" + time.formattedTime() + "], total drift = " + df.format(time.getTotalDrift()));
	}
